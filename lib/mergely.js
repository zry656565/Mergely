...
jQuery.extend(Mgly.mergely.prototype, {
	...
	init: function(el, options) {
		this.settings = {
            ...
            _resize_window_h: 0,
            _resize_window_w: 0,
            _resize_window_offset: 20,
			fgcolor: '#4ba3fa',
			bgcolor: '#eee',
			lhs: function(setValue) { },
			rhs: function(setValue) { },
			loaded: function() { },
			height: function(h) { return h - this._resize_window_offset; },
			width: function(w) { return w; },
			resize: function() {
                var h = jQuery( window ).height();
                var w = jQuery( window ).width();

                // Short Circuit Resize and fix for 20 pixel height border.
                if (h === this._resize_window_h && w === this._resize_window_w) return;
                if (this._resize_window_h !== 0) {
                    this._resize_window_offset = (this._resize_window_h - h);
                }
                this._resize_window_h = h;
                this._resize_window_w = w;
				w = jQuery(el).parent().width();
		...
		if (options.cmsettings) jQuery.extend(this.cmsettings, options.cmsettings);
	...
	bind: function() {
		...
		this.editor[this.id + '-lhs'] = CodeMirror.fromTextArea(
			jQuery('#' + this.id + '-lhs').get(0), lhs_cmsettings
		);
		...
		this.editor[this.id + '-rhs'] = CodeMirror.fromTextArea(
			jQuery('#' + this.id + '-rhs').get(0), rhs_cmsettings
		);
		// resize
		this.resizeTimeoutId = null;
        this._internalResize = function () {
			if (this.resizeTimeoutId) clearTimeout(this.resize_timeout);
			this.resizeTimeoutId = setTimeout(function () {
				...
				tthis.resizeTimeoutId = null;
			}, tthis.settings.resize_timeout);
		};

		jQuery(window).resize(this._internalResize);
	},
	unbind: function() {
    if (this.resizeTimeoutId != null) clearTimeout(this.resizeTimeoutId);
    	...
    	jQuery(window).unbind('resize', this._internalResize);
	...
	_setup: function(el) {
		...
		this.merge_right_button = jQuery(merge_right_button);
		this.merge_left_button = jQuery(merge_left_button);
		...
		jQuery(this.element).append(jQuery('<div class="mergely-margin" style="height: ' + height + '"><canvas id="' + this.id + '-lhs-margin" width="8" height="' + height + '"></canvas></div>'));
		...
		jQuery(this.element).append(jQuery('<div class="mergely-margin" style="height: ' + height + '"><canvas id="' + this.id + '-rhs-margin" width="8" height="' + height + '"></canvas></div>'));
		this.bind();
	...
	_changed: function(editor_name1, editor_name2) {
		...
		for (var i in this.change_funcs) {
      		if (!this.change_funcs.hasOwnProperty(i)) continue;
	...
	_calculate_offsets: function (editor_name1, editor_name2, changes) {
		if (this.draw_top_offset == null) {
			...
			if (!this.em_height) {
                var test = jQuery('<div class="CodeMirror-gutter-text"><pre>&nbsp;</pre></div>');
	...
	_markup_changes: function (editor_name1, editor_name2, changes) {
		...
		for (var i in changes) {
			...
			if (!this.cmsettings.readOnly) {
				...
				var tthis = this;
				jQuery(button).get(0).onclick = (function (change) {
					return function() {
